<template>
  <path
    fill-rule="evenodd"
    clip-rule="evenodd"
    d="M8.99965 16.1703L5.52965 12.7003C5.13965 12.3103 4.50965 12.3103 4.11965 12.7003C3.72965 13.0903 3.72965 13.7203 4.11965 14.1103L8.29965 18.2903C8.68965 18.6803 9.31965 18.6803 9.70965 18.2903L20.2896 7.71031C20.6796 7.32031 20.6796 6.69031 20.2896 6.30031C19.8996 5.91031 19.2696 5.91031 18.8796 6.30031L8.99965 16.1703Z"
    :fill="fillColor"
  />
</template>

<script>
export default {
  props: {
    color: { type: String, default: "#F7F9FC" },
  },
  computed: {
    fillColor: function () {
      return this.color == "" ? "#F7F9FC" : this.color;
    },
  },
};
</script>
